...
const readCards = (logFile) => {
    ...
    fs.readFile(logFile, "utf8", function(error, data) {
        ...
        var addArray = JSON.parse(data);

        for (let i = 0; i < addArray.length; i++) {
            cardArray.push(addArray[i]);
        }
    });
};
...
const quiz = (logFile, x) => {
    ...
    fs.readFile(logFile, "utf8", function(error, data) {
        ...
        if (x < addArray.length) {
            ...
            if (addArray[x].hasOwnProperty("front")) {

                var gameCard = new BasicCard(addArray[x].front, addArray[x].back);
                var gameQuestion = gameCard.front;
                var gameAnswer = gameCard.back.toLowerCase();
            } else {
                var gameCard = new ClozeCard(addArray[x].text, addArray[x].cloze);
...
//function to create a basic flashcard
const basicPrompt = [{
    name: "front",
...
//function to create a cloze flashcard
const clozePrompt = [{
    name: "text",
